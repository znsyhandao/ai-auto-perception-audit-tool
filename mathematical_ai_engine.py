...
class MathematicalAIEngine:
    ...
    def maclaurin_series_expansion(self, function_str: str, degree: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            # 计算收敛半径
            convergence_radius = self.calculate_convergence_radius(series_terms)
            
            # 找到主导项
            dominant_term = self.find_dominant_term(series_terms)
            
            # 计算级数质量
            series_quality = self.calculate_series_quality(series_terms, convergence_radius)
            ...
            return {
                ...
                "mathematical_insight": self.generate_maclaurin_insight(series_terms, convergence_radius),
    ...
    def calculate_convergence_radius(self, series_terms: List[Dict]) -> float:
        """计算级数收敛半径"""
    ...
    def find_dominant_term(self, series_terms: List[Dict]) -> Dict:
        """找到级数中的主导项"""
        if not series_terms:
    ...
    def calculate_series_quality(self, series_terms: List[Dict], convergence_radius: float) -> float:
        """计算级数质量分数 (0-1)"""
        if not series_terms:
            return 0.0
    ...
    def generate_maclaurin_insight(self, series_terms: List[Dict], convergence_radius: float) -> str:
        """生成麦克劳林级数洞察"""
        if not series_terms:
            return "Function cannot be expanded as Maclaurin series."
        
        dominant = self.find_dominant_term(series_terms)
    ...
    def taylor_series_complexity_analysis(self, execution_times: List[float], 
        ...
        try:
            ...
            # 识别主导项
            dominant_degree = self.identify_dominant_degree(coefficients)
            
            # 转换为大O表示法
            big_o = self.coefficients_to_big_o(coefficients, dominant_degree)
            
            # 计算复杂度置信度
            complexity_confidence = self.calculate_complexity_confidence(coefficients, r_squared, dominant_degree)
            
            # 生成数学证明
            mathematical_proof = self.generate_complexity_proof(coefficients, dominant_degree, r_squared)
            ...
            return {
                ...
                "mathematical_model": f"T(n) ≈ {self.format_polynomial(coefficients)}",
                "mathematical_proof": mathematical_proof,
                "fitting_quality": self.assess_fitting_quality(r_squared, residuals),
                "recommendation": self.generate_complexity_recommendation(big_o, complexity_confidence)
            }
            
        except Exception as e:
            return {"error": str(e), "analysis_type": "taylor_series"}
    
    def identify_dominant_degree(self, coefficients: np.ndarray) -> int:
        """识别多项式中的主导次数"""
        # 找到最后一个显著非零系数
    ...
    def coefficients_to_big_o(self, coefficients: np.ndarray, dominant_degree: int) -> str:
        """将系数转换为大O表示法"""
    ...
    def calculate_complexity_confidence(self, coefficients: np.ndarray, r_squared: float, dominant_degree: int) -> float:
        """计算复杂度置信度"""
        # 基于R²和系数稳定性计算置信度
        r2_score = min(1.0, r_squared * 1.2)  # 调整R²分数
        
        # 检查系数稳定性
        coeff_stability = self.assess_coefficient_stability(coefficients)
        
        # 检查主导项清晰度
        dominance_clarity = self.assess_dominance_clarity(coefficients, dominant_degree)
        
        # 综合置信度
        confidence = 0.5 * r2_score + 0.3 * coeff_stability + 0.2 * dominance_clarity
        return confidence
    
    def assess_coefficient_stability(self, coefficients: np.ndarray) -> float:
        """评估系数稳定性"""
        if len(coefficients) < 2:
            return 0.5
    ...
    def assess_dominance_clarity(self, coefficients: np.ndarray, dominant_degree: int) -> float:
        """评估主导项清晰度"""
    ...
    def generate_complexity_proof(self, coefficients: np.ndarray, dominant_degree: int, r_squared: float) -> str:
        """生成复杂度数学证明"""
        proof = []
        proof.append(f"1. Polynomial regression yields coefficients: {self.format_coefficients(coefficients)}")
    ...
    def assess_fitting_quality(self, r_squared: float, residuals) -> str:
        """评估拟合质量"""
        if r_squared >= 0.95:
            return "EXCELLENT"
        elif r_squared >= 0.85:
    ...
    def generate_complexity_recommendation(self, big_o: str, confidence: float) -> str:
        """生成复杂度建议"""
        if confidence >= 0.8:
    ...
    def format_polynomial(self, coefficients: np.ndarray) -> str:
        """格式化多项式"""
        terms = []
    ...
    def format_coefficients(self, coefficients: np.ndarray) -> str:
        """格式化系数显示"""
