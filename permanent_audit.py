            ...
            try:
                with open(changelog_file, 'r', encoding='utf-8') as f:
                    ...
                    if last_updated == today:
                        print(f"  ✓ CHANGELOG最后更新: {last_updated} (今天)")
                        evidence['CHANGELOG_fresh'] = True
                    elif last_updated:
                        print(f"  ⚠️ CHANGELOG最后更新: {last_updated} (不是今天)")
                        evidence['CHANGELOG_fresh'] = False
                    else:
                        print(f"  ⚠️ CHANGELOG最后更新日期未找到")
                        evidence['CHANGELOG_fresh'] = False
            except Exception as e:
                evidence['CHANGELOG_error'] = str(e)
                print(f"  ✗ CHANGELOG日期检查错误: {e}")
...
'''
        ...
        try:
            ...
            if result.returncode == 0 and "ALL_TESTS_PASSED" in result.stdout:
                print("  ✓ skill.py导入测试通过")
                return True, evidence
            else:
                print("  ✗ skill.py导入测试失败")
                if result.stderr:
                    print("  错误输出:")
    ...
    def check_zip_package(self):
        ...
        if 0.01 <= zip_size_mb <= 10:
            ...
            print(f"  ⚠️ ZIP包大小异常: {zip_size_mb:.2f} MB")
            return False, evidence
    ...
    def run_full_audit(self):
        ...
        if all_passed:
            ...
            print(f"\n🔧 需要修复 {total_checks-passed_checks} 个问题")
...
def main():
    """主函数"""
    if len(sys.argv) != 2:
        print("用法: python permanent_audit.py <技能路径>")
        print("示例: python permanent_audit.py D:/openclaw/releases/AISleepGen_release")
        sys.exit(1)
