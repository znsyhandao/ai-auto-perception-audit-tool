...
try {
    python -c "import fastapi; import uvicorn; print('FastAPI和Uvicorn已安装')" 2>&1
    Write-Host "依赖检查通过" -ForegroundColor Green
} catch {
    Write-Host "警告: FastAPI或Uvicorn未安装" -ForegroundColor Yellow
    Write-Host "运行: pip install fastapi uvicorn" -ForegroundColor White
}
...
# 服务配置
$services = @(
    @{Name="验证服务"; Port=8001; Path="microservices\validator-service"},
    @{Name="安全服务"; Port=8002; Path="microservices\security-service"},
    @{Name="性能服务"; Port=8003; Path="microservices\performance-service"},
    @{Name="合规服务"; Port=8004; Path="microservices\compliance-service"},
    @{Name="报告服务"; Port=8005; Path="microservices\reporting-service"},
    @{Name="监控服务"; Port=8006; Path="microservices\monitoring-service"}
)
...
foreach ($service in $services) {
    ...
    Write-Host "启动 $serviceName (端口: $port)..." -NoNewline
...
# 等待所有服务启动
Write-Host "等待服务完全启动 (10秒)..." -ForegroundColor White
...
if ($allHealthy) {
    ...
    Write-Host "部分服务启动失败，请检查日志" -ForegroundColor Yellow
}
...
Write-Host "Web管理界面: file://$PSScriptRoot/dashboard/frontend/index.html" -ForegroundColor Cyan
